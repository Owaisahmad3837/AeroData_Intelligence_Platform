...
def test_download_route_data(tmp_path):
  logger=log("Unit Test/Ingestion","route_log")

  logger.info("Starting testing.....")

  logger.info("Create temporary folder.")
  test_folder_path = tmp_path / "data/raw/route_test_data"
  logger.info(f"Temporary folder created at: {test_folder_path}")

  logger.info("Patching Path to use temporary folder.")
  with (
    patch("src.airport_data_platform.ingestion.routes.Path") as mock_path,
    patch("src.airport_data_platform.ingestion.routes.sp.run") as mock_run,
):
    mock_path.return_value=test_folder_path
    logger.info("Calling download_route_data function.")
    download_route_data()
    logger.info("download_route_data function executed successfully.")
